...
<div class="max-w-3xl mx-auto">
    <div class="mb-6 flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Update the pricing and details of this service.</p>
        </div>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <form action="{{ route('cashier.services.update', $service->id) }}" method="POST" class="p-6 md:p-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 shadow-sm transition-colors @error('name') border-red-500 @enderror" value="{{ old('name', $service->name) }}" required autofocus>
                    @error('name')
                ...
                <div>
                    ...
                    <select name="doctor_id" id="doctor_id" class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 shadow-sm transition-colors @error('doctor_id') border-red-500 @enderror">
                        ...
                        @foreach($doctors as $doc)
                            <option value="{{ $doc->id }}" {{ old('doctor_id', $service->doctor_id) == $doc->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <div class="flex rounded-xl shadow-sm">
                        ...
                        <input type="number" name="price" id="price" min="0" step="1" class="flex-1 min-w-0 block w-full rounded-none rounded-r-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 transition-colors @error('price') z-10 border-red-500 @enderror" value="{{ old('price', (int)$service->price) }}" required>
                    </div>
                    @error('price')
                        <p class="mt-2 text-sm text-red-600 flex items-center gap-1"><i data-lucide="alert-circle" class="w-4 h-4"></i> {{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 shadow-sm transition-colors @error('description') border-red-500 @enderror">{{ old('description', $service->description) }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600 flex items-center gap-1"><i data-lucide="alert-circle" class="w-4 h-4"></i> {{ $message }}</p>
                    @enderror
                </div>
